...
2. Make sure reload.sh is owned by root and it's immutable: `sudo chown root:root reload.sh` and `sudo chattr +i reload.sh`. Otherwise someone with non-root access to the system could gain root access by editing the `reload.sh` file and waiting until it's executed by the cron job. Pulling changes to `reload.sh` from the remote will fail, but that's expected and correct behaviour
